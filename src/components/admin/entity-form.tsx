...
const inputCls =
	"w-full rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5 text-sm outline-none transition-colors focus:border-[var(--color-primary)]"
...
function SubmitButton({ label }: { label: string }) {
	...
	return (
		<button
			type="submit"
			disabled={pending}
			className="rounded-[var(--radius-full)] bg-[var(--color-primary)] px-6 py-2.5 text-sm font-semibold text-white transition-opacity disabled:opacity-50"
		>
		...
		>
			...
			<div className="mt-6 flex items-center gap-3">
				...
				{cancelHref && (
					<Link
						href={cancelHref}
						className="rounded-[var(--radius-full)] border border-[var(--color-border)] px-6 py-2.5 text-sm transition-colors hover:bg-[var(--color-surface-2)]"
					>
						انصراف
					</Link>
				)}
